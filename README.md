...
- `PARENT_CHARACTER` - символ родителя. Если встречено `\p`, где `PARENT_CHARACTER = 'p'`, то это интерпретируется, как ссылка на родителя. Если `0`, то выключено. НИЧЕГО, КРОМЕ `\p` быть не должно (в пределах одного элемента)! Например, ключ `some_property1.some2.\p.inner` идентичен ключу `some_property1.inner`.
...
## Разное
```
boolean has(key); // проверяет наличие объекта по пути key. Если такого объекта нет, или он равен null, то возвращает false. Может также проверить наличие строки в массиве
