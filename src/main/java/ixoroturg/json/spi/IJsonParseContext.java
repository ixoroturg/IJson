...
class IJsonParseContext {
	...
	byte sign = 0; // Знак числа
	short exp = 0; // экспонента числа
	int[] module = new int[1]; // модуль числа
	byte modlen = 1;
	short dotPos = 0;
	...
	static IJsonParseContext openContext(InputStream reader) throws IJsonParseException {
	...
	void open(InputStream reader) throws IJsonParseException{
		...
		try{
			...
			IJsonParseException exp = new IJsonParseException("Cannot read the stream");
			exp.initCause(e);
			throw exp;
		}
	}

	int read() throws IJsonParseException{
		// int buf = BUFFER_SIZE;
