...
public class StringTest {
  public static void test(){
    ...
    try{
      ...
      Json js = IJson.ofInner(new FileInputStream("./src/testFiles/StringTest"));
      ...
      System.out.println("\nWith escape:\n");
      js = IJson.ofInner(new FileInputStream("./src/testFiles/StringTest"));
      System.out.println("Inline:\n");
      System.out.println(js);
      System.out.println("\nFormat:\n");
      System.out.println(js.toStringFormat());

      js = IJson.ofInner("{\"control_chars\": \"formfeed: \\u000c, newLine: \\u000a, Tab: \\u0009, CR: \\u000D\"}");
      ...
      js = IJson.ofInner("{\"Null string\": \"null\"}");
      ...
      try{
        js = IJson.ofInner("{\"Forbidden\": \"\\q\"}");
        System.out.println(js);
      }catch (Exception e){
        System.out.println("На \\q выдало:\n"+e.getMessage());
      }
      try{
        js = IJson.ofInner("{\"Forbidden\": \"\\p\"}");
        System.out.println(js);
      }catch (Exception e){
        System.out.println("На \\p выдало:\n"+e.getMessage());
      }
      try{
        js = IJson.ofInner("{\"Forbidden\": \"\\ something\"}");
        System.out.println(js);
      }catch (Exception e){
        System.out.println("На \\t выдало:\n"+e.getMessage());
      }
      try{
        js = IJson.ofInner("{\"Forbidden\": \"\\\" somememem \\\" \\\"\\\"\\\" \\\\\"}");
        System.out.println(js);
      }catch (Exception e){
  ...
  private static void testControl(){
      ...
      try{
        js = IJson.ofInner("{\"Forbidden\": \"\r\"}");
        System.out.println(js);
      }catch (Exception e){
        System.out.println("На \\r выдало:\n"+e.getMessage());
      }
      try{
        js = IJson.ofInner("{\"Forbidden\": \"\f\"}");
        System.out.println(js);
      }catch (Exception e){
        System.out.println("На \\f выдало:\n"+e.getMessage());
      }
      try{
        js = IJson.ofInner("{\"Forbidden\": \"\b\"}");
        System.out.println(js);
      }catch (Exception e){
