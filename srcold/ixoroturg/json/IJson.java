...
//			return result.substring(1, result.length()-1);
	...
	public Json get(Object key) {
		patternCheck:if(key instanceof String pattern) {
			if(parentJson.equals(pattern))
				return back();
			if(pattern.indexOf(separator) == -1) {
				break patternCheck;
			}
			String[] arr = pattern.split(separator);
			Json result = this;
			for(String k: arr) {
				if(parentJson.equals(k)) {
					result = result.back();
					continue;
				}
...
//		return this;
  ...
  protected String toFormatedString(int depth){
		...
		if(type == JsonType.array) {
          ...
          String s = ((IJson)js).toFormatedString(depth+1);
          if(s.length() > 1 && depth != 0 && s.indexOf(0) != '\t')
            s = "\t".repeat(depth+1) + s;
					  result += ",\n"+s;
        }
			}
		...
		String result = "";
		for(String key: map.keySet()) {
			result += ",\n" + "\t".repeat(depth+1)+"\"" +key+"\": "+((IJson)map.get(key)).toFormatedString(depth+1);
		}
		if(result.length() == 0)
			return "{}";
		return " {\n"+result.substring(2)+"\n" +"\t".repeat(depth) + "}";
